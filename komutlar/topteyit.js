...
if(!message.member.roles.cache.some(r => ['783345825345568788'].includes(r.id)) && (!message.member.hasPermission("ADMINISTRATOR")))return message.reply("Bu Komutu Kullanmak İçin Yetkiniz Bulunmamakta.")  
...
message.channel.send(new dc.MessageEmbed().setAuthor(yazı).setTimestamp().setColor("RANDOM").setFooter("Developed Phentos").setDescription(top));
